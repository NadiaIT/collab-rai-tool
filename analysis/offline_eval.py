...
import copy
import dspy
from .utils import use_lm, batch_inference, run_model, LM_DICT, get_embeddings
...
def compute_diversity_score(examples):
    embeddings = get_embeddings([example.output for example in examples])
    embeddings = np.array(embeddings)
    
    similarity_matrix = np.dot(embeddings, embeddings.T)

    pair_similarities = []
    for i in range(0, len(embeddings) // 2 - 1, 2):
...
if __name__ == "__main__":
    ...
    dataset = []
    sources = [
        ("./analysis/data/scenarios_baseline.jsonl", "baseline"),
        ("./analysis/data/scenarios.jsonl", "full"),
    ]
    ...
    for source_path, source in sources:
        with open(source_path) as f:
            ...
            for i, row in enumerate(data):
                for j, story in enumerate(row["Stories"]):
                    dataset.append(dspy.Example(
                        **{
                            input_key: row["Description"],
                        },
                        story_id=j,
                        source=source,
                        output=story["Title"] + "\n\n" + story["Story"],
                    ).with_inputs(input_key))

    with open("./analysis/data/requirements_story.json") as f:
        requirements = json.load(f)

    task_description = "Generate a story that causes potential harms to relevant stakeholders of an AI application."
    
    # eval_generator = EvalGenerator(task_description, LM_DICT["gpt-4o"], LM_DICT["gpt-4o"], max_workers=32)
    # results = eval_generator([example.toDict() for example in dataset[:3]], requirements)
    ...
    with open(f"./analysis/data/scenarios_baseline_evaluated.json", "w") as f:
        json.dump([example.toDict() for example in evaluated_dataset], f)

    all_examples = []

    for source_path, source in sources:
        with open(source_path.replace(".jsonl", "_evaluated.json")) as f:
            evaluated_dataset = json.load(f)
            evaluated_dataset = [dspy.Example(**example).with_inputs(input_key) for example in evaluated_dataset]

            for requirement in requirements:
                for example in evaluated_dataset:
                    all_examples.append({
                        "input": example.inputs().toDict()[input_key],
                        "output": example.output,
                        "requirement": requirement,
                        "source": source,
                    } | example.requirements[requirement["requirement"]])

    import pandas as pd
    df = pd.DataFrame(all_examples)
    df = df.drop("is_applicable", axis=1)
    df.to_csv("./analysis/data/evaluated_results.csv", index=False)
